...
def getResourceTreeHtml(section:bool, title:str) -> str:
    resp = requests.get(f'{host}/__structure__/text')
...
def _sendRequest(method, **parameters) -> str:
    ...
    originator = parameters.pop('originator', None)
    ...
    primitiveContent = parameters.pop('primitiveContent', None)
    if (primitiveContent) is None and method in [ requests.post, requests.put ]:
    ...
    resourceType = parameters.pop('resourceType', None)
    if (resourceType) is None and method in [ requests.post ]:
    ...
    rqi = parameters.pop('requestIdentifier', f'ri-{random.randint(1,sys.maxsize)}')
    if len(rqi) == 0:
    ...
    rvi = parameters.pop('releaseVersionIndicator', '3')
    if len(rvi) == 0:
    ...
    args = ''
    fu = parameters.pop('filterUsage', None)
    if (fu) is not None:
    ...
    filterCriteria = parameters.pop('filterCriteria', None)
    ...
    rcn = parameters.pop('resultContent', None)
    if (rcn) is not None:
    ...
    # Check verbosity
    _verbose = __verbose
    silent = parameters.pop('_silent', None)
    ...
    for i in range(repeat):
        ...
        try:
            ...
            if doOAuth:
                token = getOAuthToken(__oauthToken)
                if (token) is None:
        ...
        print(__response)
        print(__responseStatusCode)
        msg = None
        if __response is not None and "m2m:cin" in __response.keys():
          msg = __response["m2m:cin"]["con"]
        print(msg)
        return msg
...
def CREATE(**kwargs) -> None:
    ...
    """
    err = _sendRequest(requests.post, **kwargs)
    if (err) is not None:
        printHtmlError(err)
...
def RETRIEVE(**kwargs):
    ...
    """
    return _sendRequest(requests.get, **kwargs)
...
def UPDATE(**kwargs) -> None:
    ...
    """
    err = _sendRequest(requests.put, **kwargs)
    if (err) is not None:
        printHtmlError(err)
...
def DELETE(**kwargs) -> None:
    ...
    """
    err = _sendRequest(requests.delete, **kwargs)
    if (err) is not None:
        printHtmlError(err)
...
def findXPath(dct, key, default=None):
    ...
    while i < len(paths):
        if data is None:
             return default
        pathElement = paths[i]
        m = decimalMatch.search(pathElement) 
        if len(pathElement) == 0:	# return if there is an empty path element
            return default
        elif (m) is not None:	# Match array index {i}
            idx = int(m.group(1))
